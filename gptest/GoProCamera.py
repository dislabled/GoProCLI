...
class GoPro:
    ...
    def _request(self, path, param="", value="", _timeout=5, _isHTTPS=False, _context=True):
        if param != "" and value == "":
            uri = "{}{}/{}/{}".format("https://" if _isHTTPS else "http://",
                                  self.ip_addr, path, param)
            return json.dumps(requests.get(uri, timeout=_timeout, verify=_context).json())
        ...
        if self.IsRecording() == 0:
            ...
            try:
                if "FS" in self.infoCamera(constants.Camera.Firmware):
                    if "GFRNT" in folder:
                        r = requests.get(
                            "http://" + self.ip_addr + "/videos2/DCIM/" + folder + "/" + file, stream=True)
                r = requests.get(
                    "http://" + self.ip_addr + "/videos/DCIM/" + folder + "/" + file, stream=True)
                total_size = int(r.headers.get('content-length', 0)) # Total size in bytes
                block_size = 1024 # Change to kilobit
                with open(filename, 'wb') as f:
                    with alive_bar(round(total_size/block_size)) as bar:
